...
在iOS 7 ，系统不会保证后台任务一次性执行完10分钟，而是分成多个时间片执行，以便改善电池的寿命。另外，iOS 7里，有两种途径运行后台任务：background fetching 和 background notifications
...
Background notifications 其实和background fetching 的操作方式比较相似，两者都需要一些相似的步骤。
